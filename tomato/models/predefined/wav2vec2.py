...
import tomato.utils as utils
from tomato.data.data_utils import compute_mask_indices
...
class Wav2Vec2Model(nn.Module):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        # TODO: make a base model class
        if args.checkpoint_path is not None:
            print(f"===================== loading w2v2 checkpoint from {args.checkpoint_path}")
            self.load_state_dict(torch.load(args.checkpoint_path, map_location='cpu'), strict=True)
    ...
    def forward_till_encoder(self, source, padding_mask=None, mask=True, features_only=False):
        if self.feature_grad_mult > 0:
            ...
            with torch.no_grad():
                features = self.feature_extractor(source) # downsampling B*T->B*C*T'
        ...
        if padding_mask is not None:
            extra = padding_mask.size(1) % features.size(1) # padding_mask.T  % features.T'
            if extra > 0:
                padding_mask = padding_mask[:, :-extra] # remove mod part
            padding_mask = padding_mask.view(padding_mask.size(0), features.size(1), -1) # padding_mask first 2 dimention to features dimention B*T'*C'
            padding_mask = padding_mask.all(-1)

        if self.post_extract_proj is not None:
            features = self.post_extract_proj(features) # Linear proj channel size from self.embed to args.encoder_embed_dim
        ...
        if mask:
            # print("Apply Mask in wav2vec conv output...")
            ...
            # print("NOT Apply Mask in wav2vec conv output...")
            x = features
            y = unmasked_features
            mask_indices = None
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        x = self.forward_encoder(x, padding_mask)
    ...
    def extract_features(self, x, padding_mask=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, z = layer(x, self_attn_padding_mask=padding_mask, need_weights=self.need_attn)
                layer_results.append(x)
                self.attention_results.append(z) 
