...
if __name__ == "__main__":
    ...
    bs = 3
    wav_length = 64_000
    x = torch.rand(bs, wav_length)
    
    def load_wavlm_model():
        cfg = WavLMConfig()  # Use default config
        model = WavLM(cfg).to(device)
        print(f"num_layers: {cfg.encoder_layers}")
        ...
        final_feat, layer_res = features
        print(f"Final feature shape: {final_feat.shape}")
        print(f"len(layer_res): {len(layer_res)}")
        for layer_feat in layer_res:
            print(f"Layer feature shape: {layer_feat.shape}")
    load_wavlm_model()
