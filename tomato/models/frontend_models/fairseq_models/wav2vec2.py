...
#from fairseq import util # done
#from fairseq.data.data_utils import compute_mask_indices #done
#from fairseq.dataclass import ChoiceEnum, FairseqDataclass #delete
#from fairseq.distributed import fsdp_wrap #delete
#from fairseq.models import BaseFairseqModel, register_model #delete
#from fairseq.distributed.fully_sharded_data_parallel import FullyShardedDataParallel #delete
from .modules import (
    Fp32GroupNorm, #done
    Fp32LayerNorm, #done
    GradMultiply, #done
    GumbelVectorQuantizer, #done
    LayerNorm, #done
    MultiheadAttention, #done
    RelPositionalEncoding, #done
    SamePad, # done
    TransposeLast, # done
)
from .data_utils import compute_mask_indices
#from fairseq.modules.checkpoint_activations import checkpoint_wrapper # delete
#from fairseq.modules.conformer_layer import ConformerWav2Vec2EncoderLayer # done
from .modules.conformer_layer import ConformerWav2Vec2EncoderLayer
#from fairseq.modules.transformer_sentence_encoder import init_bert_params # done
#from fairseq.utils import buffered_arange, index_put, is_xla_tensor # done
import tomato.models.frontend_models.fairseq_models.utils as fairseq_utils
...
class Wav2Vec2Model(nn.Module):
    ...
    def forward(
        ...
        # print("before encoder", padding_mask.shape)
        # print(padding_mask)
        x, layer_results = self.encoder(
            x, padding_mask=padding_mask, layer=layer, corpus_key=corpus_key
        )
        # print("after encoder", padding_mask.shape)
        # print(padding_mask)
...
class TransformerEncoder(nn.Module):
    def build_encoder_layer(self, args: argparse.Namespace, **kwargs):
    ...
    def __init__(self, args: argparse.Namespace, skip_pos_conv: bool = False, override_encoder_layer: int = None):
        super().__init__()
...
if __name__ == "__main__":
    import os
    pt_path = os.getenv("XLSR_CP_PATH")
    res = torch.load(pt_path)
    state_dict = res['state_dict']
    cfg = res['cfg']
    mdl = Wav2Vec2Model(cfg)
